...
const page = async () => {
  ...
  if (!session || !session?.data || !session?.data?.doctorProfile?.id)
    return redirect("/");

  // console.log(session.data)

  const res = await fetch(
    `${process.env.SERVER_URL}/doctor/getDashInfo?userId=${session.data.doctorProfile.id}`,
    {
      method: "GET",
      headers: {
  ...
  return (
    <div className="">
      ...
      <PatientsAppointment
        datas={{
          todayAppointMentdDetails: data.todayAppointMentdDetails,
          upcomingAppointsMents: data.upcomingAppointsMents,
        }}
      />
    </div>
  );
