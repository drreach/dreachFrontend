...
//   const [selectedSlots, setSelectedSlots] = useState<string[]>([]);
  ...
  const selectedSlots = useAppSelector((state)=>state.userReducer.shedules);
  ...
  const handleAddSlot = () => {
    ...
    dispatch(addShedule(selectedTime))
      setSelectedTime("");
    }
  };

  const availableTimeSlots = timeSlots.filter((time) => !selectedSlots.includes(time));
  ...
  return (
    <div className="">
      <select className="form-select" aria-label="Default select example" value={selectedTime} onChange={handleSelectChange}>
      ...
      <button className="btn btn-success my-2 mx-2" onClick={handleAddSlot}>Add</button>
      <div>
