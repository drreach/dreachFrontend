...
function TagInput({
  ...
  handleOnSetValues: (value: string) => void;
          ...
          {values &&
            values.map((v, i) => {
              return (
                <div
                  key={i}
                  className=" flex flex-row gap-2 border border-gray-500 w-auto py-2 px-2 items-center"
                >
                  {/* add delete button also */}
                  <span className="badge bg-primary">{v}</span>
                  <span
                    className="text-red-400  font-bold text-[17px] cursor-pointer"
                    onClick={() => dispatch(removeSpecialization(i))}
                  >
                    x
                  </span>
                </div>
